...
func main() {
	...
	fmt.Printf(" Listening on 0.0.0.0:%s (accessible from network)\n", port)
	...
	fmt.Printf("   GET  http://localhost:%s/admin/users/:student_id\n", port)
	fmt.Printf("   POST http://localhost:%s/admin/promote\n", port)
	fmt.Printf("\n User endpoints:\n")
	fmt.Printf("   GET  http://localhost:%s/users/me (requires auth)\n", port)
	fmt.Printf("\n Default SuperAdmin:\n")
	fmt.Printf("   StudentID: SUPERADMIN\n")
	fmt.Printf("   Password: superadmin123\n")

	// CRITICAL: Listen on 0.0.0.0 to allow connections from network (physical devices)
